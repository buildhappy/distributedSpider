 ...
 *
 * If you wish to be notified when windows are created or pages are loaded, 
 * you need to register a WebWindowListener with the WebClient via the method 
 * WebClient.addWebWindowListener(WebWindowListener)

 * When a window is opened either by JavaScript or through the WebClient, 
 * a WebWindowEvent will be fired and passed into the WebWindowListener.webWindowOpened(WebWindowEvent) method. 
 * Note that both the new and old pages in the event will be null 
 * as the window does not have any content loaded at this point. 
 * If a URL was specified during creation of the window then the page will be loaded and another event will be fired as described below.

 * When a new page is loaded into a specific window, 
 * a WebWindowEvent will be fired and passed into the WebWindowListener.
 * webWindowContentChanged(WebWindowEvent) method.
