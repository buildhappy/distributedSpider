 ...
 * 
 * All pages are contained within WebWindow objects. 
 * This could be a TopLevelWindow representing an actual browser window, 
 * an HtmlFrame representing a <frame> element or an HtmlInlineFrame representing an <iframe> element.

 * When a WebClient is first instantiated, a TopLevelWindow is automatically created. 
 * You could think of this as being the first window displayed by a web browser. 
 * Calling WebClient.getPage(WebWindow, WebRequest) will load the new page into this window.

 * The JavaScript open() function can be used to load pages into other windows. 
 * New WebWindow objects will be created automatically by this function.
 * 
