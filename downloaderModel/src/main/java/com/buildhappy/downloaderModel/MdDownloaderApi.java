...
public class MdDownloaderApi {
	...
	private final BlockingQueue<ZMsg> downloadTask ;//= Collections.synchronizedList(new ArrayList<ZMsg>());
	private Logger LOGGER = LoggerFactory.getLogger(getClass());
	
	//private ZFrame replyTo;//返回地址
	
	public MdDownloaderApi(Socket worker , String service , boolean verbose , BlockingQueue<ZMsg> downloadTask){
		LOGGER.info("MdDownloaderApi constructor");
		assert(service != null);
	...
	void sendToBroker(MDP command , String option , ZMsg msg){
		LOGGER.info("MdDownloaderApi sendToBroker()");
	...
	public ZMsg receive(ZMsg reply){
		...
		if(reply != null){
			assert(replyTo != null);
			reply.wrap(replyTo);
			sendToBroker(MDP.REPLY , null ,reply);
			reply.destroy();
		}
		*/
		expectReply = true;
	...
	public static void main(String[] args){
		...
		MdDownloaderApi workerSession = new MdDownloaderApi("tcp://127.0.0.1:5550" , "downloader" , verbose , downloadTask);
		ZMsg reply = null;
		while(!Thread.currentThread().isInterrupted()){
